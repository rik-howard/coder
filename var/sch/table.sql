...
.  [][nk:tb.nkz] |   [nk.name] [nk.type] ([nk.length])[:exists:nk.extra] [nk.extra][] [nk.nullness],
.  []            |   primary key (id),
.  [fk:tb.fkz]   |   foreign key ([fk.name]_id) references [fk.dbName].[fk.name] (id),
.  [][uk:tb.ukz] |   unique ([pt:uk.ptz][pt.name][:isLast:pt.xount][:], [][])[:isLast:uk.xount][:],[]
.  []            | ) type = [tb.type];
.[]
